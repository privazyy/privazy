...
- `/uploads/*`: `ADMIN`, `LAWYER`, `OPERATOR`, `CLIENT`
...
- `/api/documents/generate` requires document generation role, verifies organization access, checks that the organization exists, checks that the template exists and is `ACTIVE`, and ignores any client-supplied `createdById`; the authenticated user id is used instead.
...
- `auth.login_failed`
- `auth.logout`
- `seed.admin_created` / `seed.admin_updated`
...
The landing checker result panel no longer pretends to submit a result by e-mail. It points users to the contact section until a full consented lead-capture UI with Turnstile widget is implemented. The server endpoint is already ready to accept `security.turnstileToken` from that future UI.

## Responsive Checks

`npm run responsive:check` now checks public routes by default. Private routes such as `/admin` are no longer tested as anonymous public pages because that hides the intended auth boundary and requires seeded auth/database state.

To intentionally include the private CRM route in a controlled environment, run the app with a valid database/session setup and set:

```bash
RESPONSIVE_INCLUDE_PRIVATE=true npm run responsive:check
```
...
If any value is missing or still set to `replace_in_private_env`, the seed skips safely and prints which variable names are needed. When a private seed is applied, it creates or updates the admin account and writes `seed.admin_created` or `seed.admin_updated` without storing the password. Real passwords must never be committed.
