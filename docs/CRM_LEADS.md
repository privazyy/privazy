...
Manual leads are created by staff through `/crm` or `POST /api/crm/leads`. The IOD checker creates a lead directly in its server transaction with server-owned `source=IOD_CHECKER` and `status=NEW`; it never calls the private CRM API.
