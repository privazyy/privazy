...
- [ ] Admin seed writes `seed.admin_created` or `seed.admin_updated` without storing the password.
...
- [ ] Unauthenticated users cannot view `/uploads`.
...
- [ ] `/api/documents/generate` rejects missing organizations.
- [ ] `/api/documents/generate` rejects missing or inactive templates.
...
- [ ] Landing checker UI does not show a fake "send result" action unless a real Turnstile-backed submit flow is implemented.
...
- [ ] `npm run typecheck`
- [ ] `npm run build`
- [ ] `npm run responsive:check` after starting a local server.
- [ ] `RESPONSIVE_INCLUDE_PRIVATE=true npm run responsive:check` only in an auth/database-ready environment.
