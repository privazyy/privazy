...
| --- | --- | --- | --- | --- |
| P1-006 | Secure document download/audit flow is missing. | P1 | FIXED for current `GeneratedDocument` model | Added protected endpoint, permission helper, bounded signed URL generation, and server-only RLS-protected `DocumentDownload` audit. |
...
| P1-DOC-TENANT | CLIENT document download needs tenant isolation. | P1 | FIXED for download endpoint | CLIENT access is checked against `ClientProfile.organizationId` resolved server-side before readiness, preventing both download and status-oracle leakage. |
