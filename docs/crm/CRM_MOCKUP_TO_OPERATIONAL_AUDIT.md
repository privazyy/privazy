...
Legacy prototype arrays and numeric navigation badges were removed from `crm-data.ts`. The visible navigation is limited to database-backed dashboard, leads, clients, tasks, and audit log. Server output is built in `src/server/crm/data.ts`; unsupported modules are not advertised as operational CRM. Remaining blockers are full role-matrix mutation automation, staging env verification, and production preflight.
