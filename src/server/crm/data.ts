...
export async function getCrmDatabaseData(): Promise<CrmDatabaseData> {
  ...
  const shopOrderRows: TableRow[] = orders.map((order) => {
    ...
    const invoiceStatus = order.invoices[0] ? invoiceStatusLabels[order.invoices[0].status] : "Brak";
  ...
  const invoiceRows: TableRow[] = invoices.map((invoice) => {
    ...
    return {
      cells: [
        ...
        formatMoneyCents(invoice.subtotalNetCents, invoice.currency),
        formatMoneyCents(invoice.vatCents, invoice.currency),
      ...
      secondary: invoice.externalInvoiceId ?? "mock",
      status: { label: status, tone: statusTone(status) },
    };
  });
  ...
  const lists: CrmDatabaseData["lists"] = {
    accounting: {
      action: "Podgląd faktur",
      columns: ["Nr faktury", "Klient", "Zamówienie", "Netto", "VAT", "Brutto", "Status", "Wystawiono", ""],
      emptyMessage: "Brak faktur sklepu w bazie danych.",
      icon: "Wallet",
      kpis: [
        { icon: "ReceiptText", label: "Faktury", value: String(counts.invoices), tone: counts.invoices > 0 ? "success" : "neutral" },
        { icon: "CreditCard", label: "Płatności", value: String(counts.payments), tone: counts.payments > 0 ? "brand" : "neutral" },
        { icon: "BadgeCheck", label: "Wystawione", value: String(invoices.filter((invoice) => invoice.status === "ISSUED").length), tone: "success" },
        { icon: "TriangleAlert", label: "Błędy", value: String(invoices.filter((invoice) => invoice.status === "FAILED").length), tone: invoices.some((invoice) => invoice.status === "FAILED") ? "danger" : "neutral" },
      ],
      rows: invoiceRows,
      subtitle: "Faktury sandbox z tabeli Invoice. Brak integracji live z systemem fakturowym.",
      title: "Księgowość",
    },
  ...
  const modules: Partial<Record<CrmRoute, GenericModule>> = {
    ...
    orders: makeModule({
      ...
      kpis: [
        ...
        { icon: "TriangleAlert", label: "Nieudane płatności", value: String(orders.filter((order) => order.status === "PAYMENT_FAILED").length), tone: orders.some((order) => order.status === "PAYMENT_FAILED") ? "danger" : "neutral" },
      ],
      route: "orders",
      rows: shopOrderRows,
      subtitle: "Zamówienia, płatności i faktury z fundamentu sklepu sandbox.",
      title: "Zamówienia",
    }),
    packages: makeModule({
      action: "Nowy pakiet",
