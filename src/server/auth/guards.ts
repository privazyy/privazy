...
import { ADMIN_ROLES, CRM_READ_ROLES, CRM_WRITE_ROLES, roleCanAccess, type AppRole } from "@/server/auth/roles";
...
export async function requireAdmin(options: GuardOptions = {}) {
  return requireRole(ADMIN_ROLES, options);
}

export function assertReadOnlyCannotMutate(user: AuthenticatedUser) {
  if (user.role === "READ_ONLY") {
    throw new ForbiddenError("Read-only users cannot perform mutations");
  }
}

export function assertCanMutateCrm(user: AuthenticatedUser) {
  assertReadOnlyCannotMutate(user);

  if (!roleCanAccess(user.role, CRM_WRITE_ROLES)) {
    throw new ForbiddenError("Role cannot mutate CRM data");
  }
}
