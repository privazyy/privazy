...
export function AddToCartButton({
  ...
  return (
    ...
    >
      ...
      {disabled ? "Checkout wyłączony" : state === "error" ? "Spróbuj ponownie" : state === "added" ? "W koszyku" : children}
    </Button>
  );
}
    ...
    <section className="bg-[var(--surface-page)] py-10 min-[921px]:py-14">
      <div className="grid gap-8 pvz-container min-[921px]:grid-cols-[1fr_360px]">
        <div className="min-w-0">
          ...
          {error && <p role="alert" className="mt-3 rounded-[var(--radius-md)] bg-[var(--danger-soft)] p-3 text-sm font-semibold text-[var(--red-600)]">{error}</p>}
          <div className="mt-6 grid gap-4">
            {cart.items.map((item) => {
              const isPending = pendingItemId === item.id;

              return (
                <article key={item.id} className="grid gap-4 rounded-[var(--radius-lg)] border border-[var(--border-subtle)] bg-[var(--surface-card)] p-5 shadow-[var(--shadow-sm)] min-[721px]:grid-cols-[1fr_auto]">
                  <div className="min-w-0">
...
export function CheckoutForm({ cart }: { cart: CartView }) {
  ...
  return (
    <section className="bg-[var(--surface-page)] py-10 min-[921px]:py-14">
      <div className="grid gap-8 pvz-container min-[921px]:grid-cols-[1fr_360px]">
        <form className="grid gap-5 rounded-[var(--radius-lg)] border border-[var(--border-subtle)] bg-[var(--surface-card)] p-5 shadow-[var(--shadow-sm)] min-[721px]:p-7" onSubmit={submitCheckout}>
          <div>
            <h1 className="text-[var(--fs-h1)] font-bold">Dane do zamowienia</h1>
            <p className="mt-2 text-sm leading-6 text-[var(--text-muted)]">Tworzysz zamówienie testowe. Następny ekran jawnie symuluje sukces albo błąd płatności bez pobierania środków.</p>
          </div>
          ...
          <div className="grid gap-3 rounded-[var(--radius-md)] bg-[var(--gray-50)] p-4 text-sm text-[var(--text-body)]">
            <Checkbox name="wantsInvoice" defaultChecked>Zapisz intencję otrzymania faktury. Faktura nie jest generowana w tym PR.</Checkbox>
            <Checkbox name="terms" required>
              Akceptuję robocze zasady środowiska sandbox opisane w{" "}
              <Link href={"/regulamin" as Route} className="font-semibold text-[var(--brand-ink)] underline-offset-4 hover:underline">
                placeholderze regulaminu
              </Link>
              .
            </Checkbox>
          ...
          {error && <p role="alert" className="rounded-[var(--radius-md)] bg-[var(--danger-soft)] p-3 text-sm font-semibold text-[var(--red-600)]">{error}</p>}
...
function QuantityControl({
  ...
  return (
    <div className="grid grid-cols-[40px_44px_40px] overflow-hidden rounded-[var(--radius-md)] border border-[var(--border-subtle)] bg-[var(--surface-card)]">
...
function OrderSummary({ cart, compact }: { cart: CartView; compact?: boolean }) {
  return (
    <aside className={cn("h-fit rounded-[var(--radius-lg)] border border-[var(--border-subtle)] bg-[var(--surface-card)] p-5 shadow-[var(--shadow-sm)]", compact && "min-[921px]:sticky min-[921px]:top-24")}>
